...
int main(){
	...
	while(1){
		if(sentOnce){
			printf("您可多次进行发送,要继续发送,请输入对方IP地址:\n");
		}else{
			printf("正在获取局域网内IP地址,请稍后....\n");
		}
		...
		while(!exist){
			...
			exist = 0;
			for(c = 0;c<con;c++){
				if(list[c].ip.byte1 ==ip1&&list[c].ip.byte2 ==ip2
					&&list[c].ip.byte3 ==ip3&&list[c].ip.byte4 ==ip4){
					exist = 1;
					break;
				}
			}
		...
		BYTE destmac[6];
		//目的MAC地址
		destmac[0] =  list[c].mac[0];
		destmac[1] =  list[c].mac[1];
		destmac[2] =  list[c].mac[2];
		destmac[3] =  list[c].mac[3];
		destmac[4] =  list[c].mac[4];
		destmac[5] =  list[c].mac[5];
		//赋值目的MAC地址
...
DWORD WINAPI SendArpPacket(LPVOID lpParameter) //(pcap_t *adhandle,char *ip,unsigned char *mac,char *netmask)
{
	...
	printf("自身MAC地址为:%02x-%02x-%02x-%02x-%02x-%02x\n", mac[0], mac[1], mac[2],
			mac[3], mac[4], mac[5]);
	printf("自身的IP地址为:%s\n", ip);
